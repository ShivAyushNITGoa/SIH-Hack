...
export const getReportsPage = async (
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('reports')
      .select(
        'id, title, description, category, priority, status, location, address, media_urls, user_id, assigned_to, created_at, updated_at, resolved_at'
      )
      .order('created_at', { ascending: false })
    ...
    if (error) {
      ...
      const { data: civicIssues, error: civicError } = await supabase
        .from('civic_issues')
        .select(
          'id, title, description, category, priority, status, location, address, images, reporter_id, assigned_to, created_at, updated_at, resolved_at'
        )
        .order('created_at', { ascending: false })
        .range(offset, offset + Math.max(0, limit - 1))
...
function aggregateStatsFromAny(rows: any[]) {
  ...
  return { total, pending, inProgress, resolved, closed, byStatus, byCategory, byPriority, recent }
}
