                      ...
                      <input
                        ...
                        onChange={async (e: any) => {
                          ...
                          try {
                            ...
                            try {
                              const { data: up, error: upErr } = await supabase.storage.from('media').upload(path, file, { upsert: false })
                              if (upErr) throw upErr
                              uploadResult = { data: up, bucket: 'media' }
                            } catch (mediaErr) {
                              const { data: up, error: upErr } = await supabase.storage.from('report-images').upload(path, file, { upsert: false })
                              if (upErr) throw upErr
                              uploadResult = { data: up, bucket: 'report-images' }
                ...
                <div className="space-y-6">
                  ...
                  <div className="border-t pt-6">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 text-red-600 hover:text-red-700 border border-red-300 rounded-lg hover:bg-red-50 transition-colors"
                    >
