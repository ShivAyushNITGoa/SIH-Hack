...
import { getCitizenReportById, CitizenReport, getReportComments, addReportComment, toggleReportUpvote, hasUserUpvoted, getReportUpvotesCount, ReportComment, isSubscribed, subscribeToReport, unsubscribeFromReport } from '@/lib/citizen-queries'
...
export default function IssueDetailPage() {
  ...
  const [hasUpvotedState, setHasUpvotedState] = useState<boolean>(false)
  ...
  const loadIssue = async (issueId: string) => {
    ...
    try {
      ...
      if (foundIssue) {
        setIssue(foundIssue)
        const [loadedComments, upvoteCount] = await Promise.all([
          getReportComments(issueId),
          getReportUpvotesCount(issueId),
        ])
        setComments(loadedComments)
        setUpvotes(upvoteCount)
        if (user?.id) {
          const [voted, sub] = await Promise.all([
            hasUserUpvoted(issueId, user.id),
            isSubscribed(issueId, user.id)
          ])
          setHasUpvotedState(voted)
          setSubscribed(sub)
        }
      } else {
  ...
  const handleToggleUpvote = async () => {
    ...
    try {
      ...
      const userId = user.id
      const res = await toggleReportUpvote(issue.id, userId)
      setHasUpvotedState(res.upvoted)
      setUpvotes((c) => (res.upvoted ? c + 1 : Math.max(0, c - 1)))
    } catch (error) {
      console.error(error)
      ...
      <div className="bg-white shadow-sm border-b sticky top-0 z-10">
        <div className="px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-lg font-semibold text-gray-900">NagarSetu - Issue Details</h1>
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="space-y-3">
          <button
            ...
            className={`w-full flex items-center justify-center px-4 py-3 rounded-lg transition-colors ${hasUpvotedState ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
            <ThumbsUp className="h-5 w-5 mr-2" />
            {hasUpvotedState ? 'Upvoted' : 'Upvote'} • {upvotes}
          </button>
          <button
