...
export default function IssueCard({ issue, onStatusChange, onViewDetails, departments = [], onDepartmentAssigned, duplicates = [], onMarkDuplicate }: IssueCardProps) {
  ...
  const triage = suggestTriage({ title: issue.title, description: issue.description, category: issue.category as any })
  ...
  return (
    <div className="card hover:shadow-md transition-shadow">
      ...
      {issue.status === 'in_progress' && issue.assigned_to && (
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-200">
        <div className="flex items-center space-x-2">
          ...
          {issue.status === 'in_progress' && hasCompletion && (
            <button
              onClick={() => onStatusChange(issue.id, 'resolved')}
